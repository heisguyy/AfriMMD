...
# configuration_vision_encoder_decoder import VisionEncoderDecoderConfig
...
class VisionEncoderDecoderModel(PreTrainedModel, GenerationMixin):
    ...
    def __init__(
        ...
        # Add language model head and tie with decoder embeddings
        self.lm_head = nn.Linear(
            config.decoder.d_model, config.decoder.vocab_size, bias=False
        )
        # Initialize from decoder's embeddings
        self.lm_head.weight = self.decoder.embed_tokens.weight
        # Explicitly tie the weights
        self._tie_or_clone_weights(self.lm_head, self.decoder.embed_tokens)
    ...
    def from_pretrained(
        ...
        if from_tf:
            ...
            tf_model = TFVisionEncoderDecoderModel(
            ...
            with tempfile.TemporaryDirectory() as tmpdirname:
                ...
                if hasattr(model, "enc_to_dec_proj"):
                    ...
                    model.enc_to_dec_proj.bias.data = (
                        enc_to_dec_proj_bias.contiguous()
                    )
    ...
    def forward(
        ...
        **kwargs,
        ...
        if not return_dict:
            if loss is not None:
                return (loss,) + (logits + decoder_outputs[1:]) + encoder_outputs
            else:
